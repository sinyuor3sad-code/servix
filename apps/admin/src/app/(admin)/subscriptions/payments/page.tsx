...
const ST: Record<PayStatus, { label: string; icon: React.ElementType; cls: string }> = {
  paid:      { label: 'مكتمل',  icon: CheckCircle, cls: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' },
  pending:   { label: 'معلّق',  icon: Clock,       cls: 'bg-amber-500/10 text-amber-400 border-amber-500/18' },
  overdue:   { label: 'متأخر',  icon: XCircle,     cls: 'bg-red-500/8 text-red-400 border-red-500/15' },
          ...
          <>
            <table className="w-full text-[13px]">
              <thead><tr className="border-b border-white/[0.05]">
                {['رقم الفاتورة', 'الصالون', 'المبلغ', 'تاريخ الاستحقاق', 'تاريخ الدفع', 'الحالة', ''].map((h, i) => (
                  <th key={i} className="px-5 py-4 text-start text-[11px] font-bold tracking-widest text-white/20">{h}</th>
                ))}
              </tr></thead>
              <tbody>
                {invoices.map((inv, i) => {
                  const status = (inv.status || 'pending') as PayStatus;
                  const st = ST[status] || ST.pending;
                  const StI = st.icon;
                  return (
                    <tr key={inv.id} className={`group transition-colors hover:bg-white/[0.02] ${i < invoices.length - 1 ? 'border-b border-white/[0.03]' : ''}`}>
                      <td className="px-5 py-3.5 font-mono text-[12px] font-bold text-violet-400/70">{inv.invoiceNumber}</td>
                      <td className="px-5 py-3.5 font-bold text-white/75 group-hover:text-white">{inv.tenantName || '—'}</td>
                      <td className="px-5 py-3.5 font-bold text-amber-400/65" style={TN}>{(inv.total ?? inv.amount ?? 0).toLocaleString()} <span className="text-[10px] text-white/15">ر.س</span></td>
                      <td className="px-5 py-3.5 text-white/30" style={TN}>{inv.dueDate ? new Date(inv.dueDate).toLocaleDateString('ar-SA', { month: 'short', day: 'numeric' }) : '—'}</td>

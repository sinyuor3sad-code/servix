...
export class WhatsAppEvolutionWebhookController {
   ...
   *   1. opt-out keyword → record opt-out (highest priority)
   *   2. sender == manager phone → ManagerReplyHandler (before AI)
   *   3. else → AIReceptionService (fire-and-forget)
   */
  private async handleIncomingMessage(
    ...
    // ────────────────────────────────────────────
    // [2] MANAGER REPLY — before AI processing
    // ────────────────────────────────────────────
    if (text) {
    ...
    // ────────────────────────────────────────────
    // [3] AI RECEPTION — fire-and-forget
    // ────────────────────────────────────────────
    ...
    // Check AI reception setting
    const aiEnabled = await this.isAIReceptionEnabled(tenantDb);
    if (!aiEnabled) return;

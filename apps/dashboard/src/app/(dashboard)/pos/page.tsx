...
export default function POSPage(): React.ReactElement {
  ...
  const payMutation = useMutation({
    mutationFn: async (method: string) => {
      ...
      if (!selectedEmployee) throw new Error('اختر الموظفة');
      ...
      await dashboardService.recordInvoicePayment(
        invoice.id,
        { amount: total, method: method as 'cash' | 'card' | 'bank_transfer' },
        accessToken!,
      );
    ...
    onError: (err: Error) => {
      toast.error(err.message || 'حدث خطأ أثناء إنشاء الفاتورة');
    },
  });

  function handlePay(method: string): void {
    if (cart.length === 0) {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      <div className="flex items-center justify-between">
        <Link
          href="/pos/quick"
          className="inline-flex items-center gap-2 rounded-xl bg-[var(--brand-primary)] px-4 py-2.5 text-sm font-medium text-white transition-colors hover:opacity-90"
        >
      ...
      <div className="flex flex-1 flex-col gap-4 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          {loadingCategories ? (
            ...
            <EmptyState title="لا توجد خدمات" description="جرّب تغيير التصنيف أو البحث" />
          ) : (
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
              {filteredServices.map((service) => (
                <button
                  ...
                  className="flex flex-col items-start gap-1 rounded-xl border border-[var(--border)] bg-[var(--background)] p-4 text-start transition-all hover:border-[var(--brand-primary)] hover:shadow-sm active:scale-[0.98]"
                >
                  ...
                  <span className="mt-auto text-sm font-bold text-[var(--brand-primary)]">
        ...
        <CardContent className="flex flex-1 flex-col gap-4 overflow-hidden">
            ...
            {showWalkInForm ? (
              ...
              <div className="relative">
                <input
                  ...
                  className="flex h-9 w-full rounded-lg border border-[var(--border)] bg-[var(--background)] px-3 py-1 text-sm text-[var(--foreground)] placeholder:text-[var(--muted-foreground)] focus:outline-none focus:ring-2 focus:ring-[var(--ring)]"
                />
                <Button
                  variant="outline"
                  size="sm"
                ...
                {clientsData?.items.length && clientSearch.length >= 2 ? (
                  <div className="absolute start-0 end-0 top-full z-10 mt-1 rounded-lg border border-[var(--border)] bg-[var(--background)] shadow-lg">
                    {clientsData.items.map((c) => (
                      <button
                        ...
                        className="flex w-full items-center gap-2 px-3 py-2 text-start text-sm hover:bg-[var(--muted)]"
                      >
                        <span className="font-medium">{c.fullName}</span>
                        <span className="text-xs text-[var(--muted-foreground)]">{c.phone}</span>
                      </button>
                    ))}
                  </div>
                ) : null}
              ...
              cart.map((item) => (
                ...
                >
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium">{item.service.nameAr}</p>
          ...
          <div className="space-y-2 border-t border-[var(--border)] pt-3">
            ...
            <div className="flex items-center gap-2">
              <span className="shrink-0 text-sm text-[var(--muted-foreground)]">الخصم</span>
              <Input
                type="number"
                value={discountInput}
            ...
            <div className="flex justify-between text-base font-bold">
              <span>الإجمالي</span>
              <span className="text-[var(--brand-primary)]">{formatCurrency(total)}</span>
            </div>
          </div>

          ...
          {e.cart.length > 0 && (
            <div className={`shrink-0 ${brd(4)} border-t px-3.5 py-2.5 space-y-2`}>
              ...
              <div className="flex items-center gap-1">
                ...
                {e.couponApplied ? (
                  <button onClick={e.removeCoupon} className={`${BS} rounded-lg px-2 py-1.5 text-[8px] font-bold text-red-400 hover:bg-red-500/10`}><X size={10} /></button>
                ) : (
                  <button onClick={e.applyCoupon} disabled={!e.couponCode.trim() || e.couponMut.isPending} className={`${BS} rounded-lg px-2.5 py-1.5 text-[8px] font-bold disabled:opacity-30 ${T}`} style={accentColor}>{e.couponMut.isPending ? '...' : 'تطبيق'}</button>
                )}
              </div>

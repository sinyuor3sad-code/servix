        ...
        <aside className={`hidden w-[420px] shrink-0 flex-col border-s ${brd(4)} md:flex ${G1}`}>
          ...
          {e.cart.length > 0 && (
            <div className={`shrink-0 border-t ${brd(4)} p-4 space-y-3`}>
              <div className="flex items-center gap-1.5">
                <Percent size={12} className="shrink-0 text-emerald-400" style={{ opacity: 0.5 }} />
                <input type="number" value={e.globalDisc} onChange={ev => e.setGlobalDisc(ev.target.value)} placeholder="خصم" className={`w-20 rounded-xl border ${brd(6)} ${bg(3)} px-3 py-2.5 text-[12px] text-center text-[var(--foreground)] focus:outline-none ${T}`} style={TN} />
                <div className={`flex overflow-hidden rounded-xl border ${brd(6)}`}><button onClick={() => e.setGlobalDiscType('fixed')} className={`${BS} px-3 py-2.5 text-[10px] font-bold ${e.globalDiscType === 'fixed' ? 'text-black' : 'text-[var(--muted-foreground)]'}`} style={e.globalDiscType === 'fixed' ? accentBg : undefined}>ر.س</button><button onClick={() => e.setGlobalDiscType('percentage')} className={`${BS} px-3 py-2.5 text-[10px] font-bold ${e.globalDiscType === 'percentage' ? 'text-black' : 'text-[var(--muted-foreground)]'}`} style={e.globalDiscType === 'percentage' ? accentBg : undefined}>%</button></div>
              </div>
              {/* Coupon */}
              <div className="flex items-center gap-1.5">
                ...
                {e.couponMut.isPending && <span className="h-4 w-4 animate-spin rounded-full border-2 border-[var(--brand-accent)]/30 border-t-[var(--brand-accent)]" />}
                {e.couponApplied && <button onClick={e.removeCoupon} className={`${BS} rounded-xl px-3 py-2.5 text-[10px] font-bold text-red-400 hover:bg-red-500/10`}><X size={12} /></button>}
              </div>
          ...
          <div className="fixed inset-x-0 bottom-0 z-50 max-h-[80vh] rounded-t-3xl bg-[var(--background)] border-t border-[var(--border)] shadow-2xl overflow-y-auto md:hidden animate-fade-in-up" style={{ paddingBottom: 'max(1rem, var(--safe-bottom, 0px))' }}>
            ...
            <div className="px-4 pt-2 space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                {e.couponMut.isPending && <span className="h-4 w-4 animate-spin rounded-full border-2 border-[var(--brand-accent)]/30 border-t-[var(--brand-accent)]" />}
                {e.couponApplied && <button onClick={e.removeCoupon} className={`${BS} rounded-xl px-3 py-2.5 text-[11px] font-bold text-red-400 hover:bg-red-500/10`}><X size={14} /></button>}
              </div>
            </div>

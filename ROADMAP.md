# 🗺️ SERVIX Roadmap — خطة التنفيذ الرسمية

> ⚡ هذا الملف هو المرجع الأساسي لخطة التنفيذ. يتم تحديثه بعد إنجاز كل مهمة.
> آخر تحديث: 5 أبريل 2026
> الحالة: المنصة حيّة في الإنتاج مع ~70% من الميزات

---

## 📊 نسبة الإنجاز العامة

```
المرحلة 1 [██░░░░░░░░] 0%  — الحماية والاستقرار
المرحلة 2 [░░░░░░░░░░] 0%  — تفعيل الميزات الجاهزة
المرحلة 3 [░░░░░░░░░░] 0%  — التكاملات الخارجية
المرحلة 4 [░░░░░░░░░░] 0%  — التقارير والذكاء
المرحلة 5 [░░░░░░░░░░] 0%  — تطبيق الجوال
المرحلة 6 [░░░░░░░░░░] 0%  — DevOps والتوسع
```

---

## 🔴 المرحلة 1: الحماية والاستقرار (2-3 أيام)

> ⚠️ **حرجة** — بدونها بيانات العملاء معرضة للخطر!

### 1.1 📁 النسخ الاحتياطي التلقائي
**الحالة:** ❌ لم يُبدأ | **الأولوية:** 🔴 حرجة | **الوقت:** ~3 ساعات

- [ ] إنشاء سكربت `backup.sh` يعمل `pg_dump` لكل قواعد البيانات
- [ ] جدولة عبر `cron` كل 6 ساعات
- [ ] تخزين النسخ في MinIO (S3 المحلي)
- [ ] الاحتفاظ بآخر 7 أيام (حذف القديم تلقائياً)
- [ ] إنشاء سكربت `restore.sh` للاسترجاع

**الملفات:**
```
tooling/docker/scripts/backup.sh       (جديد)
tooling/docker/scripts/restore.sh      (جديد)
tooling/docker/docker-compose.prod.yml (إضافة cron container)
```

---

### 1.2 🛡️ Rate Limiting
**الحالة:** ❌ لم يُبدأ | **الأولوية:** 🔴 حرجة | **الوقت:** ~2 ساعة

- [ ] تفعيل `@nestjs/throttler` في الـ API
- [ ] 100 طلب/دقيقة للمستخدم العادي
- [ ] 10 محاولات/دقيقة لتسجيل الدخول
- [ ] 5 محاولات/دقيقة لإنشاء الحسابات
- [ ] استخدام Redis كمخزن للعدادات

**الملفات:**
```
apps/api/src/app.module.ts
apps/api/src/modules/auth/auth.controller.ts
apps/api/package.json
```

---

### 1.3 🧹 تنظيف التنانت التجريبيين
**الحالة:** ❌ لم يُبدأ | **الأولوية:** 🟡 مهمة | **الوقت:** ~1 ساعة

- [ ] تحديد التنانت التجريبيين
- [ ] حذف قواعد البيانات الفارغة/التجريبية
- [ ] حذف سجلاتهم من `platform` DB
- [ ] التأكد من أن الـ API لا يحاول الاتصال بهم

---

## 🟡 المرحلة 2: تفعيل الميزات الجاهزة (5-7 أيام)

> هذه الميزات بنيتها الأساسية جاهزة (Schema + صفحات) — تحتاج ربط وتفعيل فقط

### 2.1 📦 المخزون (Inventory)
**الحالة:** ❌ لم يُبدأ | **الأولوية:** 🟡 مهمة | **الوقت:** ~8 ساعات

- [ ] ربط صفحة المخزون بالـ API (CRUD للمنتجات)
- [ ] ربط فئات المنتجات
- [ ] حركات المخزون (إضافة/صرف/تعديل)
- [ ] ربط منتج بخدمة (خصم تلقائي عند تقديم الخدمة)
- [ ] تنبيه نفاد المخزون

**Models جاهزة:** `Product`, `ProductCategory`, `InventoryMovement`, `ServiceProduct`

---

### 2.2 ❤️ نظام الولاء (Loyalty)
**الحالة:** ❌ لم يُبدأ | **الأولوية:** 🟡 مهمة | **الوقت:** ~6 ساعات

- [ ] تحديد نسبة النقاط (1 ريال = 1 نقطة)
- [ ] إضافة نقاط تلقائياً عند إتمام الموعد
- [ ] صفحة استبدال النقاط بخصم
- [ ] عرض رصيد النقاط في صفحة العميل
- [ ] إعدادات الولاء في الإعدادات

**Models جاهزة:** `LoyaltyPoints`, `LoyaltyTransaction`

---

### 2.3 🧾 ZATCA (الفوترة الإلكترونية)
**الحالة:** ❌ لم يُبدأ | **الأولوية:** 🟡 مهمة (مطلب حكومي) | **الوقت:** ~8 ساعات

- [ ] إعداد شهادة ZATCA (CSR + CSID)
- [ ] توليد QR Code للفاتورة المبسطة
- [ ] تنسيق XML حسب معيار UBL 2.1
- [ ] ربط مع Sandbox أولاً ثم Production

**Models جاهزة:** `ZatcaCertificate`, `ZatcaInvoice`
**⚠️ يحتاج تسجيل رسمي في بوابة فاتورة هيئة الزكاة والدخل**

---

### 2.4 🧬 Client DNA (ذاكرة العميل)
**الحالة:** ❌ لم يُبدأ | **الأولوية:** 🟢 تحسين | **الوقت:** ~4 ساعات

- [ ] إضافة تاب "DNA" في صفحة العميل
- [ ] حفظ: تفضيلات، حساسيات، ملاحظات
- [ ] عرض ملاحظات العميل عند إنشاء موعد جديد
- [ ] تحديث تلقائي بعد كل زيارة

**Model جاهز:** `ClientDna`

---

### 2.5 📋 الباقات (Packages)
**الحالة:** ❌ لم يُبدأ | **الأولوية:** 🟢 تحسين | **الوقت:** ~4 ساعات

- [ ] صفحة إنشاء باقة (مجموعة خدمات بسعر مخفض)
- [ ] ربط باقة بموعد
- [ ] عرض الباقات في الكاشير/POS

**Models جاهزة:** `Package`, `PackageService`

---

## 🔵 المرحلة 3: التكاملات الخارجية (4-6 أيام)

> تفتح مصادر إيرادات جديدة + تحسّن تجربة العميل

### 3.1 💳 بوابة الدفع الإلكتروني
**الحالة:** ❌ لم يُبدأ | **الأولوية:** 🔵 عالية | **الوقت:** ~12 ساعة

**خيارات مقترحة:**
| البوابة | الأفضل لـ | الرسوم |
|---------|-----------|--------|
| **Moyasar** | السعودية (مدى + Apple Pay) | 2.1% |
| **Tap Payments** | الخليج | 2.5% |
| **Stripe** | عالمي | 2.9% |

- [ ] إنشاء حساب في البوابة المختارة
- [ ] إضافة `payments` module في الـ API
- [ ] صفحة الدفع في صفحة الحجز (Booking)
- [ ] Webhook لتأكيد الدفع
- [ ] إيصال إلكتروني

---

### 3.2 💬 تكامل واتساب
**الحالة:** ❌ لم يُبدأ | **الأولوية:** 🔵 عالية | **الوقت:** ~8 ساعات

- [ ] إنشاء حساب WhatsApp Business API
- [ ] تصميم Templates (تذكير موعد، تأكيد حجز، شكر)
- [ ] إنشاء `whatsapp.service.ts`
- [ ] ربط مع نظام الإشعارات
- [ ] إرسال تلقائي: تأكيد + تذكير + شكر

---

### 3.3 📱 SMS
**الحالة:** ❌ لم يُبدأ | **الأولوية:** 🟢 متوسطة | **الوقت:** ~4 ساعات

- [ ] التكامل مع مزود SMS سعودي (Unifonic/Taqnyat)
- [ ] إرسال OTP للتحقق
- [ ] تذكير بالموعد

---

## 📊 المرحلة 4: التقارير المتقدمة والذكاء (4-6 أيام)

### 4.1 📈 تقارير إضافية
**الحالة:** ❌ لم يُبدأ | **الوقت:** ~8 ساعات

- [ ] صفحة تقرير الخدمات (الأكثر طلباً، الأعلى إيراداً)
- [ ] صفحة تقرير المصروفات (شهري مع مقارنة)
- [ ] تقرير الربح الصافي (إيرادات - مصروفات - ديون)
- [ ] تصدير التقارير PDF/Excel

---

### 4.2 💰 التسعير الذكي (Dynamic Pricing)
**الحالة:** ❌ لم يُبدأ | **الوقت:** ~8 ساعات

- [ ] ربط صفحة التسعير بالـ API
- [ ] زيادة سعر عطلة نهاية الأسبوع
- [ ] خصم أوقات الهدوء
- [ ] أسعار موسمية (رمضان/أعياد)
- [ ] تطبيق تلقائي عند إنشاء موعد

**Model جاهز:** `PricingRule`

---

### 4.3 📢 نظام التسويق (Campaigns)
**الحالة:** ❌ لم يُبدأ | **الوقت:** ~8 ساعات

- [ ] ربط صفحة التسويق بالـ API
- [ ] إنشاء حملة (اسم + رسالة + جمهور)
- [ ] إرسال عبر واتساب/SMS
- [ ] تتبع نتائج الحملة

**Model جاهز:** `Campaign`

---

## 📱 المرحلة 5: تطبيق الجوال (2-3 أسابيع)

### 5.1 تطبيق العميل (React Native / Expo)
**الحالة:** ❌ هيكل فقط | **الوقت:** ~2 أسبوع

- [ ] تسجيل دخول / OTP
- [ ] البحث عن صالون
- [ ] عرض خدمات الصالون
- [ ] حجز موعد
- [ ] تتبع حالة الموعد
- [ ] رصيد الولاء
- [ ] الفواتير السابقة
- [ ] الإشعارات
- [ ] الملف الشخصي

---

## ⚙️ المرحلة 6: DevOps والتوسع (5-7 أيام — بالتوازي)

### 6.1 🔄 CI/CD Pipeline
**الحالة:** ❌ لم يُبدأ | **الوقت:** ~8 ساعات

- [ ] GitHub Actions: Lint + Type-check عند كل PR
- [ ] Build Docker images تلقائي
- [ ] Auto-deploy عند merge إلى `main`
- [ ] Rollback تلقائي عند فشل Health check

---

### 6.2 📊 Monitoring & Alerting
**الحالة:** ❌ لم يُبدأ | **الوقت:** ~8 ساعات

- [ ] تفعيل Sentry (Error tracking)
- [ ] Prometheus + Grafana
- [ ] تنبيهات: CPU/RAM/Disk
- [ ] Dashboard مراقبة الأداء

---

### 6.3 🔐 تحسينات أمنية
**الحالة:** ❌ لم يُبدأ | **الوقت:** ~8 ساعات

- [ ] 2FA (Two-Factor Authentication)
- [ ] OAuth (Google login)
- [ ] Session management محسّن
- [ ] Audit logging
- [ ] تشفير البيانات الحساسة

---

## 📅 الجدول الزمني

| المرحلة | المدة | الأولوية |
|---------|-------|----------|
| **1. الحماية** | 2-3 أيام | 🔴 فوري |
| **2. التفعيل** | 5-7 أيام | 🟡 أسبوع |
| **3. التكامل** | 4-6 أيام | 🔵 أسبوعين |
| **4. الذكاء** | 4-6 أيام | 🟢 شهر |
| **5. الجوال** | 2-3 أسابيع | 🟢 شهرين |
| **6. DevOps** | 5-7 أيام | 🟡 بالتوازي |

**الإجمالي:** ~6-8 أسابيع

...
logs = run('docker logs servix-admin --tail 5 2>&1')
print(f'5. {logs}')

# Test
for p in ['/manifest.json', '/sw.js', '/login', '/icons/icon-192.png']:
    code = run(f'curl -s -o /dev/null -w "%{{http_code}}" -k https://127.0.0.1{p} -H "Host: admin.servi-x.com" 2>&1')
    print(f'   {p}: HTTP {code}')

# Check icon size (should be real image not placeholder)
out = run('curl -s -k https://127.0.0.1/icons/icon-192.png -H "Host: admin.servi-x.com" 2>&1 | wc -c')
print(f'   icon size: {out} bytes')

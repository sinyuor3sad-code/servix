...
### A8-IV-033 — merge `chore/e8-prod-hardening` → `feature/ai-reception-phases-1-8`

Resolves the 3-way fork between `origin/main`, `feature/ai-reception-phases-1-8` (prod-live, 27+ commits ahead of main, never pushed to origin until this card), and `chore/e8-prod-hardening` (29 commits of session 1-4 hardening work).

**Approach (Option B — single merge commit):**

Cherry-picking all 29 chore commits one-by-one onto the feature branch would have produced ~10 conflict resolutions and high cognitive risk. A single `git merge --no-ff` produces one resolution pass; the per-commit history is fully preserved on the chore branch and surfaces via `git log --first-parent` once merged.

**Step A — Cleaned up the feature branch on prod first:**

Prod's `/root/servix` had been sitting in an unfinished `cherry-pick ca4c542` state since 2026-05-14 (CHERRY_PICK_HEAD present, 3 working-tree intents mid-flight). The original commit `ca4c542` bundled three changes; owner split them per his guidance into two separate commits to preserve intent:

- `50108c5` — `chore: remove legacy SERVIX_خطة_التنفيذ.md (superseded by docs/principal-audit/execution/)`
- `c426e8f` — `docs(ai-reception): replace obsolete plans with new integration plan`

`git cherry-pick --abort` cleared the dangling state; the AI doc was snapshotted to `/tmp` first then re-staged for commit 2 to preserve content. Five total commits pushed from prod to `origin/feature/ai-reception-phases-1-8` (the 3 pre-existing prod-only commits plus these two).

**Step B — Push prod feature branch to origin:**

Authentication was missing on the prod host (HTTPS remote with no token), so I `git bundle`-d the commits on prod, `scp`-d the bundle to local, `git fetch`-d the bundle into the local feature branch, and pushed from local. Net effect: same as a direct prod push.

**Step C — Merged chore into feature locally with 4 conflicts resolved:**

| File | Conflict type | Resolution |
|---|---|---|
| `.env.example` | UU | `--theirs` — chore's `<REQUIRED_NO_DEFAULT>` placeholders (V-29) supersede feature's `change-me-*` defaults; no feature-only keys to preserve |
| `apps/api/Dockerfile` | UU | Hand-merged: kept feature's Chromium-for-Puppeteer apk block (PDF rendering) **and** appended chore's `tzdata` (A8-IV-023) into the same `apk add` |
| `CLAUDE.md` | AA | Hand-merged: chore's English structure as base; added an "AI Reception V2" section preserving feature's file map; added an "A8-IV-033 state note" warning that prod currently deploys from feature, not main, until this PR lands; linked `docs/principal-audit/execution/e8-changelog.md` from the reference list |
| `tooling/docker/docker-compose.prod.yml` | UU | `--theirs` — chore's content is what's actually running on prod (it's the result of every session 2/3/4 sed/scp edit) |

**Step C.5 (unexpected) — A8-IV-044: cherry-pick abort silently overwrote prod's compose file:**

The `git cherry-pick --abort` + subsequent `git checkout --` calls in Step A reset prod's `/root/servix/tooling/docker/docker-compose.prod.yml` to the feature-branch baseline (size 21112, pre-chore content). Running containers were unaffected (env vars and mounts had been baked in at container-create time during sessions 2/3/4), but the next `docker compose up -d` operation would have recreated containers with the rolled-back config — a latent breakage.

Detected via SHA mismatch between local merged file (chore content) and prod on-disk file:
```
local:  fbfc8d4dfb0e233a6147ab1a74931d9daba92546cd8f77ad74667636b2e6d40f
prod:   ee144e56387682e7f5e33343794212fbefc2356be55257a6a8894bc90591a866  ← rolled-back
```

Recovery: backed the rolled-back file to `.bak-A8-IV-033-overwrite-20260518_053643`, then `scp`'d the local merged file (which has the correct chore content) back into place. Post-recovery SHAs match (`fbfc8d4...`). All 20 containers preserved their `Up` state; `docker compose config --quiet` clean.

**Followup ticket:**

- **`A8-IV-044`** (P3 process): never run `git` operations directly on prod `/root/servix` during active development. Cherry-pick aborts, merges, and `git checkout --` can silently revert working-tree files that other ops (sed/scp/compose recreate) have been authoritatively modifying. Future strategy: clone the repo locally, do all git work there, and only `scp` finished file content to prod. The bundle-out / scp-in pattern used in Step B is the correct shape.

**Verification (post-recovery, pre-merge-commit):**

| Check | Result |
|---|---|
| 4/4 conflicts resolved, no `<<<<<<<` markers anywhere | ✓ |
| `docker compose -f tooling/docker/docker-compose.prod.yml config --quiet` (local) | clean |
| local SHA == prod SHA (`fbfc8d4...`) | ✓ |

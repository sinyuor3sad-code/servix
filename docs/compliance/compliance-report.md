...
|------|----------|--------|---------|
| حق الوصول (PDPL Art. 14) | `GET /api/v1/data-rights/my-data` | ✅ | يصدّر بيانات المستخدم الفعلية (ملف شخصي + مواعيد + فواتير + سجل نشاط) |
| حق التصحيح (PDPL Art. 15) | `PATCH /api/v1/data-rights/my-data` | ✅ | يعدّل البيانات في جدولَي User (منصة) و Client (مستأجر) |
| حق الحذف (PDPL Art. 16) | `DELETE /api/v1/data-rights/my-data` | ✅ | طلب حذف مع فترة انتظار 30 يوم، محفوظ في قاعدة البيانات |
| إلغاء طلب الحذف | `PATCH /api/v1/data-rights/my-data/cancel-deletion` | ✅ | إلغاء خلال فترة الانتظار |
| حق النقل | `GET /api/v1/data-rights/my-data` | ✅ | JSON export شامل |
| فترة انتظار 30 يوم | DataRightsService.requestDeletion | ✅ | محفوظة في Settings table مع cron job للتنفيذ |

> **التأمين:** الهوية تُستخرج من JWT (لا يمكن للمستخدم الوصول لبيانات غيره). البيانات الضريبية (الفواتير + سجلات التدقيق) محفوظة حسب نظام ZATCA.
